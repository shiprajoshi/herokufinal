...
export default ({ getState, dispatch }) => next => action => {
  if (action.type === ROUTER_DID_CHANGE) {
    ...
    const promise = new Promise((resolve) => {
      const doTransition = () => {
        ...
        Promise.all(getDataDependencies(components, getState, dispatch, location, params, true))
          ...
          .catch(error => {
            ...
            console.warn('Warning: Error in fetchDataDeferred', error); // eslint-disable-line
            return resolve();
          });
      };
      ...
      Promise.all(getDataDependencies(components, getState, dispatch, location, params))
        ...
        .catch(error => {
          ...
          console.warn('Warning: Error in fetchData', error); // eslint-disable-line
          return doTransition();
        });
    });
    ...
    if (__SERVER__) {
      ...
      getState().router = promise; // eslint-disable-line no-param-reassign
    }
