 ...
 * NOTE: Any options, requiring special handling (e.g. 'stream'),
 *       that encounter an error will be removed from the options.
 */
function getOptions() {
  ...
  if (_.has(options, 'stream')) {
    try {
      ...
      if (_.has(options, 'stream.rotatingLogs') && options.stream.rotatingLogs.active) { //eslint-disable-line

        if (options.stream.rotatingLogs.fileName.length && options.stream.directoryPath.length) { //eslint-disable-line
          ...
          throw new Error('An invalid fileName or directory. Path was provided for the rotating logs option.'); //eslint-disable-line
        }
      } else {
        // create the WriteStream to use for the logs
        if (options.stream.fileName.length && options.stream.directoryPath.length) { //eslint-disable-line
